...
extension Event{
    ...
    var calendarStartDate:DateComponents{
        return Calendar.current.dateComponents([.day,.month,.year,.hour,.minute],from: start_timestamp)
    }
    
    var calendarEndDate:DateComponents{
        return Calendar.current.dateComponents([.day,.month,.year,.hour,.minute],from: end_timestamp)
    }
}
